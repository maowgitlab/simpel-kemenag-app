...
use App\Mail\ApplicantStatusUpdatedMail;

class CmsController extends Controller
{
    protected $media, $category, $tag,
    $user, $userDetail, $listService,
    $service, $serviceApplicant, $comment;

    public function __construct(Media $media, Category $category, Tag $tag, User $user, UserDetail $userDetail, ListService $listService, Service $service, ServiceApplicant $serviceApplicant, Comment $comment)
    {
        $this->media = $media;
    ...
    public function media()
    {
        return view('cms.pages.media.index', [
            'medias' => $this->media->select('id', 'category_id', 'judul', 'slug', 'konten', 'gambar', 'penting', 'jumlah_dibaca')->with('category', 'tags')
    ...
    {
        return view('cms.pages.media.create', [
    ...
    public function mediaStore(Request $request)
    {
        ...
        $path = 'img/media';
        $filename = $this->resizeAndSaveImage($image, $path, 854, 480, 75);
    ...
    public function editMedia($slug)
    {
        ...
        $categories = $this->category->select('id', 'nama_kategori')->get();
        $tags = $this->tag->select('id', 'nama_tag')->get();
        return view('cms.pages.media.edit', compact('categories', 'tags', 'media'));
    }
    ...
    {
        return view('cms.pages.profile', [
            'user' => $this->user->with('userDetail')->find(auth()->user()->id),
        ]);
    }
    ...
    public function profileUpdate(Request $request, $id)
    {
        ...
        if ($request->hasFile('profil')) {
            if ($user->profil == 'profil.png') {
                ...
                unlink(storage_path('app/public/' . $user->profil));
                $image = $request->file('profil');
                $path = 'img/user';
    ...
    public function tag()
    {
        return view('cms.pages.tag.index', [
    ...
    {
        return view('cms.pages.tag.edit', [
            'tag' => $this->tag->findOrFail($id),
        ]);
    }
    ...
    public function user()
    {
        return view('cms.pages.user.index', [
            'users' => $this->user->with('userDetail')->where('role', '!=', 'admin')->orderByDesc('created_at')->get(),
        ]);
    }
    ...
    {
        $user = $this->user->findOrFail($id);
        if ($user->userDetail->profil != 'profil.png') {
            unlink(storage_path('app/public/' . $user->userDetail->profil));
        }
        $user->delete();
    ...
    public function userStore(Request $request)
    {
        ...
        $this->userDetail->create([
            'user_id' => $this->user->latest()->first()->id,
            'email' => $data['email'],
        ]);
    ...
    public function updateUser(Request $request, $id)
    {
        $data = $request->validate([
            ...
            'email' => 'required|unique:user_details,email,' . $id,
            'role' => 'required|in:admin,penulis',
            'status' => 'required|in:aktif,non-aktif',
        ], [
            'username.required' => 'Username harus diisi.',
        ...
        $this->userDetail->where('user_id', $user->id)->update([
            'email' => $data['email'],
        ]);
    ...
    {
        return view('cms.pages.report.view.all-media', [
    ...
    public function mediaByCategory(Request $request)
    {
        if ($request->has('categoryID')) {
            ...
            return view('cms.pages.report.view.media-by-category', [
                'medias' => $medias,
    ...
    public function mediaByTime(Request $request)
    {
        if ($request->has('start') && $request->has('end')) {
            ...
            return view('cms.pages.report.view.media-by-time', compact('medias', 'start', 'end'));
        } else {
            $medias = null;
            $start = null;
            $end = null;
    ...
    {
        return view('cms.pages.report.view.popular-media', [
    ...
    {
        return view('cms.pages.report.view.most-media-category', [
    ...
    {
        return view('cms.pages.services.listService.index', [
    ...
    {
        return view('cms.pages.services.listService.create');
    }
    ...
    {
        return view('cms.pages.services.listService.edit', [
    ...
    {
        return view('cms.pages.services.service.index', [
    ...
    {
        return view('cms.pages.services.service.create', [
            'listServices' => $this->listService->latest()->get(),
        ]);
    }
    ...
    public function serviceStore(Request $request)
    {
        ...
        $message = $this->generateSuccessMessage('Service berhasil dibuat.');
    ...
    {
        return view('cms.pages.services.service.edit', [
            'service' => $this->service->findOrFail($id),
            'listServices' => $this->listService->latest()->get(),
        ]);
    }
    ...
    {
        return view('cms.pages.services.inbox.index', [
    ...
    public function sendStatusEmail(Request $request)
    {
        ...
        if ($applicant) {
            Mail::to($applicant->email)->send(new ApplicantStatusUpdatedMail($applicant));
    ...
    {
        return view('cms.pages.report.view.service-in', [
            'inboxes' => $this->serviceApplicant->with('list', 'service')->latest()->get(),
        ]);
    }
    ...
    {
        return view('cms.pages.report.view.service-category', [
    ...
    {
        return view('cms.pages.report.view.service-status', [
    ...
    {
        return view('cms.pages.comment.index', [
