    ...
    <div class="card-body">
      ...
      <div class="pt-4 pb-2">
        ...
        <p class="small">Silahkan generate token dibawah ini untuk aktivasi akun anda yang akan dikirim ke email:
          <u>{{ substr($user->userDetail->email, 0, 1) . str_repeat('*', strpos($user->userDetail->email, '@') - 2) . substr($user->userDetail->email, strpos($user->userDetail->email, '@') - 1) }}</u>
        </p>
      </div>
        ...
        <div class="col-12">
          ...
          <div class="input-group">
            ...
            <button type="button" id="generateButton" class="btn btn-light input-group-text"
              data-email="{{ $user->userDetail->email }}">Generate</button>
            @error('token')
