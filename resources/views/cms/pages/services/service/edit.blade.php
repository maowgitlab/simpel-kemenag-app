  ...
  <section class="section">
    <div class="row">
      <div class="col-md-12">
        <div class="card">
          <div class="card-body">
            <form class="row g-3 mt-2" action="{{ route('updateService', $service->id) }}" method="POST" enctype="multipart/form-data">
              @csrf
              @method('PUT')
              <div class="mb-3">
                ...
                @enderror">
                  ...
                  @foreach ($listServices as $listService)
                    <option value="{{ $listService->id }}" {{ $listService->id == $service->list_id ? 'selected' : '' }}>{{ $listService->judul }}</option>
                  @endforeach
                </select>
                @error('list_id')
